...
public static class ProcessorPluginLoader
{
    ...
    public static List<ProcessorPluginInfo> LoadAll(string directory, ProcessorRegistry registry)
    {
        ...
        // Sort DLLs so that XmlToJson loads before JsonPrettify.
        // The correct pipeline order is: XML → XmlToJson → JsonPrettify → file.
        // We sort by the ProcessorOrder attribute when present, otherwise fall back
        // to a name-based heuristic: XmlToJson* < JsonPrettify* < everything else.
        var dlls = Directory.GetFiles(directory, "*.dll")
            ...
            .ThenBy(path => path); // stable alphabetic within the same priority
